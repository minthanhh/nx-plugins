...
import type { ArrayLiteralExpression, Node, PropertyAssignment, ReturnStatement } from 'typescript';

export type Target = 'build' | 'serve' | 'test' | 'preview';
export type TargetFlags = Partial<Record<Target, boolean>>;

export function ensureAstroConfigIsCorrect(
  tree: Tree,
  path: string,
  imports: string[] = [],
  integrations: string[],
  ...
  updatedContent = handlePluginNode(updatedContent ?? fileContent, imports, integrations) ?? updatedContent;
...
function handleBuildOrTestNode(
  ...
  if (buildOrTestNode.length) {
    return tsquery.replace(updatedFileContent, `PropertyAssignment:has(Identifier[name="${name}"])`, (node: PropertyAssignment) => {
      const existingProperties = tsquery.query(node.initializer, 'PropertyAssignment') as PropertyAssignment[];
...
function transformCurrentBuildObject(
  ...
  const { tsquery } = require('@phenomnomnominal/tsquery');
  const currentBuildObject = tsquery.query(returnStatements[index], 'ObjectLiteralExpression')?.[0].getText();

  const currentBuildObjectStart = returnStatements[index].getStart();
  const currentBuildObjectEnd = returnStatements[index].getEnd();
  const newReturnObject = tsquery.replace(returnStatements[index].getText(), 'ObjectLiteralExpression', (_node: Node) => {
    return `{
        ...${currentBuildObject},
...
function transformConditionalConfig(conditionalConfig: Node[], appFileContent: string, buildConfigObject: {}): string | undefined {
  const { tsquery } = require('@phenomnomnominal/tsquery');
  ...
  const returnStatements: ReturnStatement[] = tsquery.query(ifStatement[0], 'ReturnStatement');
...
function handlePluginNode(appFileContent: string, imports: string[], integrations: string[]): string | undefined {
  const { tsquery } = require('@phenomnomnominal/tsquery');
  const file = tsquery.ast(appFileContent);
  const integrationsNode = tsquery.query(file, 'PropertyAssignment:has(Identifier[name="integrations"])');

  let writeFile = false;

  if (integrationsNode.length) {
    appFileContent = tsquery.replace(file.getText(), 'PropertyAssignment:has(Identifier[name="integrations"])', (node: Node) => {
      const found = tsquery.query(node, 'ArrayLiteralExpression') as ArrayLiteralExpression[];
      let updatedIntegrationsString = '';

      const existingIntegrationNodes = found?.[0].elements ?? [];

      for (const integration of existingIntegrationNodes) {
        updatedIntegrationsString += `${integration.getText()},\n`;
      }

      for (const integration of integrations) {
        if (!existingIntegrationNodes?.some((node) => node.getText().includes(integration))) {
          updatedIntegrationsString += `${integration},\n`;
        }
      }

      return `integrations: [${updatedIntegrationsString}]`;
    });
    writeFile = true;
  } else {
          ...
          appFileContent = applyChangesToString(appFileContent, [
            {
              type: ChangeType.Insert,
              index: propertyAssignments[0].getStart(),
              text: `integrations: [${integrations.join(',\n')}],`,
            },
          ]);
          writeFile = true;
        } else {
